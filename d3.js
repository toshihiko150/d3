!function() {
  ...
  var d3_svg_symbols = d3.map({
    ...
    star: function(size) {
      var r = Math.sqrt(size / d3_svg_symbolSqrt3), inr = r * Math.sin(π / 10) / Math.sin(7 * π / 10), output = "M";
      function x(val) {
        return (val % 2 == 0 ? r : inr) * Math.cos(val * π / 5 - π / 2);
      }
      function y(val) {
        return (val % 2 == 0 ? r : inr) * Math.sin(val * π / 5 - π / 2);
      }
      for (var i = 0; i < 10; i++) {
        if (i != 0) output += "L";
        output += x(i) + "," + y(i);
